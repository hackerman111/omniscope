...
```
[x] Добавить FolderType enum в omniscope-core
[x] Добавить FilePresence enum
[x] Обновить BookCard: добавить file_presence, folder_id, virtual_folder_ids
[x] SQLite миграция: таблицы folders, book_virtual_folders
[x] SQLite: ALTER TABLE books ADD COLUMN folder_id, file_presence
[x] Индексы: idx_folders_parent, idx_books_folder
[x] Юнит-тесты: CRUD для folders в БД
[x] Тест: BookCard roundtrip с FilePresence::NeverHadFile
...
```
[x] FolderTree::build() — из SQLite, < 50ms для 1000 папок
[x] FolderTree::apply_change() — инкрементальные обновления
[x] FolderSync::full_scan() — diff диска и БД
[x] FolderSync::apply_sync() — применить с выбранной стратегией
[x] FolderOps::create_folder() — mkdir + БД + дерево
[x] FolderOps::rename_folder() — mv + обновить дочерние пути
[x] FolderOps::move_folder() — mv + обновить всё поддерево
[x] FolderOps::delete_folder() — KeepFiles и WithFiles режимы
[x] FolderOps::move_book_to_folder() — mv файла + обновить БД
[x] CLI: omniscope folder create/rename/move/delete/tree/info --json
[x] CLI: omniscope sync --dry-run --json
...
```
[x] LibraryWatcher::start() — notify интеграция
[x] Обработка: NewBookFile, BookFileRemoved, DirectoryCreated, DirectoryRemoved
[x] EventDebouncer — 2000ms задержка
[x] Размерный фильтар (min_file_size_kb)
[x] Интеграция в TUI event loop: handle_next_event()
[x] Уведомление в статус-баре при новом файле
[x] CLI: omniscope watch start/stop/status --json
[x] Конфиг: [watcher] в library.toml
...
```
[x] LeftPanelMode enum: LibraryView / FolderTree
[x] FolderTreePanel::render() с иконками, отступами, счётчиками
[x] Ghost count индикатор: "34+2○"
[x] Раздел Virtual Folders
[x] Tab: переключение LibraryView ↔ FolderTree
[x] FOLDER mode state machine (отдельный от Normal)
[x] FOLDER mode навигация: j/k/h/l, gg/G, za/zo/zc/zR/zM
[x] FOLDER mode: a (create), r (rename), dd (delete), m/p (move)
[x] Inline rename: поле ввода прямо в дереве, Esc/Enter
[x] Visual mode: V + d/m операторы на папках
[x] Статус-бар в FOLDER mode: "FOLDER  rust/   34 books"
[x] / поиск по именам папок
...
```
[x] CenterPanelMode enum: BookList / FolderView
[x] gv: переключить BookList ↔ FolderView
[x] FolderContents: подпапки + книги в виде единого списка
[x] Хлебные крошки (breadcrumb) в заголовке центральной панели
[x] Enter на папке → войти; - → выйти в родителя
[x] Ghost books: dim стиль + индикатор "○ ghost"
[x] DetachedBooks: orange ⚠ индикатор
[x] Сортировка: FoldersFirst / Mixed / BooksFirst (T переключение)
[x] Все vim операторы работают на смешанных папка/книга элементах
[x] Preview в правой панели: папка → статистика, книга → карточка
[x] FolderViewSort в конфиге
...
```
[x] SyncReport отображение в TUI (Sync Panel)
[x] :sync / @sync открывает панель
[x] Визуальная классификация: NEW / DETACHED / UNTRACKED DIRS
[x] Опции для каждого элемента: [Import] [Ignore] [Relink] [Keep as ghost]
[x] Кнопка [Apply all] с preview + подтверждением
[x] Watcher уведомления: импортировать панель новых файлов
[x] Ghost books: gf → "Find & Download" через Sci-Hub/Anna's
[x] Ghost books: E → attach existing file picker
[x] CLI: omniscope book list --ghost/--detached --json
[x] CLI: omniscope book attach/locate/detach --json
