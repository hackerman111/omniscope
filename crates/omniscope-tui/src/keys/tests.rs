...
fn run_keys(app: &mut App, sequence: &str) {
    for c in sequence.chars() {
        if c == '|' { continue; } // visual separator only
...
fn test_G_default_goes_to_bottom() {
    ...
    run_keys(&mut app, "G");
    assert_eq!(app.selected_index, app.books.len() - 1, "G should go to last line");
}
...
fn test_space_space_easy_motion() {
    ...
    assert!(matches!(app.popup, Some(crate::popup::Popup::EasyMotion(_))), 
...
fn test_space_slash_easy_motion_pending() {
    ...
    if let Some(crate::popup::Popup::EasyMotion(ref state)) = app.popup {
        assert!(state.pending, "EasyMotion should be in pending state after Space /");
    }
}
...
fn test_delete_undo_restores() {
    ...
    if let Some(entry) = app.undo_stack.last() {
        assert!(matches!(entry.action, omniscope_core::undo::UndoAction::UpsertCards(_)),
...
fn test_normal_mode_hints() {
    ...
    assert!(hints.iter().any(|h| h.key.contains("j")), "Should have j/k hint");
    assert!(hints.iter().any(|h| h.key.contains("d")), "Should have delete hint");
    assert!(hints.iter().any(|h| h.key.contains("u")), "Should have undo hint");
}
...
fn test_sidebar_gg_G() {
    ...
    run_keys(&mut app, "G");
    assert_eq!(app.sidebar_selected, app.sidebar_items.len() - 1, "G should move sidebar to bottom");
}
...
fn test_pending_0_motion() {
    ...
    assert!(app.books.len() < initial_len, "d0 should delete items from start to current");
...
fn test_enter_in_booklist() {
    ...
    assert!(!app.status_message.is_empty(), "Enter in BookList should set status message");
}
...
fn test_undo_key_works() {
    ...
    run_keys(&mut app, "u");
    assert!(app.status_message.contains("Nothing to undo"), "u should trigger undo function");
}
...
fn test_visual_G_goes_bottom() {
    ...
    run_keys(&mut app, "vG");
    assert_eq!(app.mode, Mode::Visual);
    assert_eq!(app.selected_index, app.books.len() - 1, "G in visual should go to bottom");
    assert_eq!(app.visual_selections.len(), app.books.len(), "Should select all items");
}
...
fn test_refresh_books_preserves_cursor() {
    ...
    assert_eq!(app.books[app.selected_index].id, book_id,
...
fn test_cs_cycles_status() {
    ...
    assert_ne!(format!("{:?}", initial_status), format!("{:?}", new_status),
...
fn test_cr_opens_rating_popup() {
    ...
    assert!(matches!(app.popup, Some(crate::popup::Popup::SetRating { .. })),
...
fn test_cy_opens_year_edit_popup() {
    ...
    assert!(matches!(app.popup, Some(crate::popup::Popup::EditYear { .. })),
        "cy should open EditYear popup, not AddBook");
}
...
fn test_ca_opens_authors_edit_popup() {
    ...
    assert!(matches!(app.popup, Some(crate::popup::Popup::EditAuthors { .. })),
        "ca should open EditAuthors popup, not AddBook");
}
...
fn test_visual_c_does_not_delete() {
    ...
    assert_eq!(app.books.len(), initial_len,
...
fn test_visual_add_tag_operator() {
    ...
    assert!(matches!(app.popup, Some(crate::popup::Popup::AddTagPrompt { .. })),
