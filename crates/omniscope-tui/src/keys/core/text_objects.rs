...
pub fn get_text_object_range(app: &App, object: char, kind: TextObjectKind) -> Option<Vec<usize>> {
    ...
    match object {
        ...
        'l' => {
            ...
            if let Ok(card) = omniscope_core::storage::json_cards::load_card_by_id(&cards_dir, &current_book.id) {
                ...
                let indices: Vec<usize> = app.books.iter().enumerate()
                    .filter(|(_, b)| {
                        if let Ok(c) = omniscope_core::storage::json_cards::load_card_by_id(&cards_dir, &b.id) {
        ...
        't' => {
            ...
            if current_tags.is_empty() { return None; }
