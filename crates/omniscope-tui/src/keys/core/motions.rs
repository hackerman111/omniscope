use crate::app::{App, CenterPanelMode};

fn get_max_index(app: &App) -> usize {
    if app.center_panel_mode == CenterPanelMode::FolderView {
        app.center_items.len().saturating_sub(1)
    } else {
        app.books.len().saturating_sub(1)
    }
}
