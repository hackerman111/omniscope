...
fn render_left_zone(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let content = Line::from(vec![
        Span::styled(" 󰂺 omniscope ", Style::default().fg(app.theme.frost_ice()).add_modifier(Modifier::BOLD)),
        Span::styled(" › ", Style::default().fg(app.theme.muted())),
        Span::styled(filter_text, Style::default().fg(app.theme.frost_mint())),
    ]);

    frame.render_widget(Paragraph::new(content).style(Style::default().bg(app.theme.bg_secondary())), area);
}
...
fn render_mode_zone(frame: &mut Frame, app: &App, area: Rect) {
    let (label, bg, fg) = match app.mode {
        ...
        Mode::Visual | Mode::VisualLine | Mode::VisualBlock => (" VISUAL ", app.theme.orange(), app.theme.bg()),
    ...
    let content = Line::from(Span::styled(label, Style::default().bg(bg).fg(fg).add_modifier(Modifier::BOLD)));
    frame.render_widget(Paragraph::new(content).alignment(ratatui::layout::Alignment::Center), area);
}
...
fn render_right_zone(frame: &mut Frame, app: &App, area: Rect) {
    ...
    if let Some(reg) = app.vim_register {
        spans.push(Span::styled(format!(" \"{} ", reg), Style::default().fg(app.theme.yellow())));
    }
    
    if !app.marks.is_empty() {
        spans.push(Span::styled(" m. ", Style::default().fg(app.theme.frost_mint())));
    }
    
    // AI Indicator
    spans.push(Span::styled(" ●AI ", Style::default().fg(app.theme.purple())));
