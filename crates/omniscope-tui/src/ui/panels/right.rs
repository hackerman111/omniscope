...
pub fn render(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let content = if let Some(book) = app.selected_book() {
        ...
        {
            article_card::build_preview_lines(
                card,
                usize::from(area.width.saturating_sub(2)),
                &app.theme,
            )
        } else {
            render_default_preview(book, area, app)
        }
    } else {
        vec![
            Line::from(""),
...
fn render_default_preview(book: &BookSummaryView, area: Rect, app: &App) -> Vec<Line<'static>> {
    ...
    vec![
        ...
        Line::from(Span::styled(
            format!(
                "  {}",
                "─".repeat(usize::from(area.width.saturating_sub(6)).max(1))
            ),
            Style::default().fg(app.theme.border()),
        )),
        Line::from(""),
