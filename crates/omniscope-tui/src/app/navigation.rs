...
impl App {
    ...
    pub fn move_next_group(&mut self) {
        ...
        if let Some(_next_idx) = self.sidebar_items.iter().enumerate()
            .skip(current + 1)
            .position(|(_, item)| matches!(item, super::SidebarItem::Library{..} | super::SidebarItem::TagHeader | super::SidebarItem::FolderHeader))
        {
    ...
    pub fn move_prev_group(&mut self) {
        if self.sidebar_selected == 0 { return; }
